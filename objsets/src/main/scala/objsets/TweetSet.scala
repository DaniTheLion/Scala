...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    if (p(elem)) {
      val l = left.filterAcc(p, acc incl elem)
      right.filterAcc(p, l)
    }
	else {
  ...
  override def mostRetweeted: Tweet = {
    var temp = elem
    foreach(x => if (x.retweets > temp.retweets) temp = x)
    temp
  }
  ...
  def descendingByRetweetAcc(acc: TweetList): TweetList = {
    val twit = mostRetweeted
    new Cons(twit, this.remove(twit).descendingByRetweetAcc(Nil))
  }
